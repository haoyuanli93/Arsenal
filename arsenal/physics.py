...
def get_intensity_polarization_effect(pixel_position_m, reference_position_m, polarization):
    """
    This function calculate the polarization correction for each pixel

    This correction is

            |np.cross(pixel direction, polarization)|^2 = 1 - np.square(np.dot(pixel direction, polarization))

    One should notice that this effect only applies for the intensity because I have
    introduced the square. Because if one consider the field, then the polarization factor should contain
    phase which can not be derived from this result.
