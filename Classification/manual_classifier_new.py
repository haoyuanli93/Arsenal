...
def load_image_function():
    ...
    pattern = PhotoImage(master=mainframe,
                         file=os.path.abspath(image_files[image_index])).zoom(3)
...
def load_data_function():
    ...
    # Save the image files list to disk
    with open(input_folder + '/file_list.pkl', 'wb') as fp:
        pickle.dump(image_files, fp)
